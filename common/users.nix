{config, ...}: {
  sops.secrets."root_user_password" = {
    format = "yaml";
    sopsFile = ../secrets/all/secrets.yaml;
    neededForUsers = true;
  };

  users.mutableUsers = false;
  users.users = {
    root.hashedPasswordFile = config.sops.secrets."root_user_password".path;
    pacman = {
      isNormalUser = true;
      extraGroups = ["wheel"];
      openssh.authorizedKeys.keys = [
        "lovis@mars sk-ecdsa-sha2-nistp256@openssh.com AAAAInNrLWVjZHNhLXNoYTItbmlzdHAyNTZAb3BlbnNzaC5jb20AAAAIbmlzdHAyNTYAAABBBNyocYkpTw5qwoN946klQxYlM9vEk1fpmdqgQJlzSGCx8iwNcEuOHTNCQmcXwOX0aa77bv9FTbPXoHWuMzlsuH4AAAAEc3NoOg== token2"
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIFZd7G6ancBJP9NRmIjlPhy7LTPcRm3zNK77NyhcYMQ4 lovis@mars"
      ];
    };
    e1mo = {
      isNormalUser = true;
      extraGroups = ["wheel"]; # Enable ‘sudo’ for the user.
      openssh.authorizedKeys.keys = [
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIBfbb4m4o89EumFjE8ichX03CC/mWry0JYaz91HKVJPb e1mo"
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAID9x/kL2fFqQSEyFvdEgiM2UKYAZyV1oct9alS6mweVa e1mo (ssh_0x6D617FD0A85BAADA)"
      ];
    };
    ruru = {
      isNormalUser = true;
      extraGroups = ["wheel"];
      openssh.authorizedKeys.keys = [
        "ssh-rsa  linus@3-Anaklet"
      ];
    };
    servermensch = {
      isNormalUser = true;
      extraGroups = ["wheel"];
      openssh.authorizedKeys.keys = [
        "ssh-rsa  jetzt"
      ];
    };
    tarva = {
      isNormalUser = true;
      extraGroups = ["wheel"];
      openssh.authorizedKeys.keys = [
        "ssh-rsa "
      ];
    };
  };
}
